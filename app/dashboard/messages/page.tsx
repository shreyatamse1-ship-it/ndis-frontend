...
export default function MessagesPage() {
    ...
    useEffect(() => {
        ...
        if (role === "participant") {
            router.push("/dashboard/participant/messages");
            return;
        }
        ...
        <div className="min-h-screen bg-slate-100 px-4 py-6">
            <div className="mx-auto max-w-350">
                <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
                    <div className="mb-4 text-sm uppercase tracking-[0.24em] text-slate-500 font-semibold">Support Worker Messages</div>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h1 className="text-3xl font-semibold text-slate-900">Messages</h1>
                            <p className="mt-2 max-w-2xl text-sm text-slate-500">Reach out to participants and keep all chat in one place.</p>
                        </div>
                        <div className="rounded-3xl bg-slate-50 px-4 py-2 text-sm text-slate-600 border border-slate-200">Live support chat</div>
                    </div>
                </div>
                                ...
                                users
                                    ...
                                    .map((user) => (
                                        ...
                                        >
                                            ...
                                            <div className="mt-1 text-sm text-slate-500">{user.role || "Participant"}</div>
                                        </button>
                                    ))
                            )}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto bg-slate-100 px-6 py-6 space-y-4">
                            {!receiverId ? (
                                <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-8 text-slate-600 text-sm">
                                    Select a participant to begin messaging.
                                </div>
                            ) : messages.length === 0 ? (
                                <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-8 text-slate-600 text-sm">
                                ...
                                messages.map((msg) => (
                                    ...
                                    >
                                        <div className={`max-w-[75%] rounded-3xl px-5 py-4 text-sm leading-6 ${msg.sender_id === userId ? "bg-blue-600 text-white" : "bg-white text-slate-900 border border-slate-200"}`}>
                                            <div>{msg.message}</div>
                                            {msg.created_at && (
